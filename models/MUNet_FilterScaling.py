...
class MUNet_FilterScaling(nn.Module):

    def __init__(self, in_channels, init_features, latent_dim, out_channels=1):
        super(MUNet_FilterScaling, self).__init__()
...
class BasicConv3d(nn.Module):
    ...
    def forward(self, x, lowRes):
        x = F.conv3d(x, self.conv.weight * self.fs(lowRes), bias=None, padding=self.padding)
        x = self.bn(x)
        x = self.relu(x)

        x = F.conv3d(x, self.conv2.weight * self.fs2(lowRes), bias=None, padding=self.padding)
        x = self.bn2(x)
        x = self.relu2(x)
