...
class MUNet_Hypernetworks(nn.Module):

    def __init__(self, in_channels, init_features, latent_dim, out_channels=1):
        super(MUNet_Hypernetworks, self).__init__()
...
class FilterScaling(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, latent_dim):
        super().__init__()
        ...
        self.style = nn.Linear(latent_dim, self.out_channel * self.in_channel * self.kernel_size[0] * self.kernel_size[1] * self.kernel_size[2])

    def forward(self, hyperparameter):
        latent_fea = self.mapping(hyperparameter)

        out = self.style(latent_fea).squeeze().reshape(self.out_channel, self.in_channel, self.kernel_size[0], self.kernel_size[1], self.kernel_size[2])
        return out
...
class BasicConv3d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, latent_dim, kernel_size, padding):
        ...
        self.fs = FilterScaling(in_channels, out_channels, kernel_size, latent_dim)
        self.bn = nn.InstanceNorm3d(out_channels)
        self.relu = nn.PReLU(out_channels)

        self.fs2 = FilterScaling(out_channels, out_channels, kernel_size, latent_dim)
        self.bn2 = nn.InstanceNorm3d(out_channels)
        self.relu2 = nn.PReLU(out_channels)

    def forward(self, x, lowRes):
        x = F.conv3d(x, self.fs(lowRes), bias=None, padding=self.padding)
        x = self.bn(x)
        x = self.relu(x)

        x = F.conv3d(x, self.fs2(lowRes), bias=None, padding=self.padding)
        x = self.bn2(x)
        x = self.relu2(x)
